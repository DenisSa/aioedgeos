import asyncio
from aiohttp import ClientSession, CookieJar, ClientTimeout, WSMsgType
...
class TaskEvery:
    def __init__(self, call, *args, interval=0, sync_once=None, offset=0, **kwargs):
        ...
        if sync_once is None and not offset:
            sync_once = True
        if not call:
...
class EdgeOS:
    ...
    async def is_logged_in(self):
        with suppress():
            if await self.sys_info() is not None:
                return True
        return False
    ...
    async def ping(self, target='1.1.1.1', count=3, size=100):
        ret = ''
        init = {'SUBSCRIBE': [
            {'name': 'ping-feed', 'sub_id': f'ping-{target}', 'target': target, 'count': count, 'size': size}]}
    ...
    async def stats(self,
        ...
        if subs is None:
            subs = ["export", "discover", "interfaces", "system-stats", "num-routes", "config-change", "users"]
        if reload_on_change and 'config-change' not in subs:
            subs.append('config-change')
        init = {'SUBSCRIBE': [{'name': x} for x in subs]}
        async for payload in self._ws(init=init):
            with suppress(KeyError):
                if reload_on_change and 'config-change' in payload \
                        and payload.get('config-change', [])['commit'] == 'ended':
            ...
            try:
                ...
                async with AsyncExitStack() as stack:
                    ...
                    ws = await stack.enter_async_context(
                        self.session.ws_connect(f"{self.url}/ws/stats", headers=self.headers, origin=self.url,
                                                ssl=self.ssl))
                    await stack.enter_async_context(TaskEvery(ws_ping, ws, interval=30, sync_once=False))
                    ...
                    while True:
                        ...
                        if msg.type != WSMsgType.TEXT:
                            logging.debug(
                                f"got non text websocket data {msg.data!r} "
                                f"this probbaly means the socket was closed so let's start a fresh one")
                            break
